...
- A `utilities/` module are autoloaded unless its name end with `-builder`, which is 
reserved for standalone modules (see [iso-builder](./utilities/iso-builder.nix))
...
`system.nix` and `generated.nix` (`hardware-configuration.nix`)
